...
for idx=companies
    ...
    for year=years
        ...
        for t=1:T
            ...
            %    betaT.*(kappaT+1)./(alphaT.*kappaT)/scale, ...
            %    2 * alphaT);
            %R(1:length(predprobs),t+1) .* predprobs
        ...
        set(ax(2),'ylim',[min(X) max(X)+5]);
        set(ax,{'ycolor'},{'k';'k'})
        y=ylabel(ax(2), 'AAPL daily price', 'Color','black'); % label left y-axis
        %ylabel
        ...
        %set(ax(1),'ylim',[min(maxes(:,1)) max(maxes(:,1))])
        set(ax,{'ycolor'},{'k';'k'})
        y=ylabel(ax(2), 'AAPL daily price', 'Color','black')
        xlabel(ax(1), 'Day'); % label left y-axis
        %set(ax(1),'YDir','reverse');
